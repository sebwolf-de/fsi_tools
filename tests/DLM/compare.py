...
for k in range(1,N+1):
    ...
    print('CN, max_iter, mean_iter, min_iter:')
    it = np.count_nonzero(res > 0, 1)
    print('      ', np.max(it), np.mean(it), np.min(it))
    time_CN[k-1] = step_time
    iter_CN[k-1] = np.max(it)
    ...
    print('TR, max_iter, mean_iter, min_iter:')
    it = np.count_nonzero(res > 0, 1)
    print('      ', np.max(it), np.mean(it), np.min(it))
    time_TR[k-1] = step_time
    iter_TR[k-1] = np.max(it)
...
print('BE     &$'+str(int(iter_BDF1[1]))+'$&$'+str(int(iter_BDF1[2]))+'$&$'+str(int(iter_BDF1[3]))+'$&$'+str(int(iter_BDF1[4]))+'$\\\\')
print('BDF-$2$&$'+str(int(iter_BDF2[1]))+'$&$'+str(int(iter_BDF2[2]))+'$&$'+str(int(iter_BDF2[3]))+'$&$'+str(int(iter_BDF2[4]))+'$\\\\')
print('CN     &$'+str(int(iter_CN[1]))+'$&$'+str(int(iter_CN[2]))+'$&$'+str(int(iter_CN[3]))+'$&$'+str(int(iter_CN[4]))+'$\\\\')
print('TR     &$'+str(int(iter_TR[1]))+'$&$'+str(int(iter_TR[2]))+'$&$'+str(int(iter_TR[3]))+'$&$'+str(int(iter_TR[4]))+'$\\\\')
