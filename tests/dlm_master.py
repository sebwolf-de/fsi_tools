...
def assemble_blokwise_force(ux_n,uy_n,xs_n,ys_n):
...
def assemble_blokwise_matrix():
    mat1 = sparse.hstack([A,-BT])
...
A = sparse.vstack([
    ...
    sparse.hstack( [sparse.csr_matrix((ndofs_u,ndofs_u)), A22] )
    ])
...
for cn_time in range(0,len(ph.stampa)):
    ...
    mat = assemble_blokwise_matrix()

    force = assemble_blokwise_force(ux_n,uy_n,xs_n,ys_n)
